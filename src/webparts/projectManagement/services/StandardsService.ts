...
export class StandardsService {
  ...
  public async uploadNewStandards(file: IFilePickerResult,nextVersion: string, clientName: string, projectNumber: string): Promise<void> {
    try {
      ...
      await this.sp.web.getFolderByServerRelativePath(versionFolder)
  ...
  public async GetCurrentVersion(clientName: string): Promise<string> {
    const clientFolder = `${this.STANDARDS_LIBRARY}/${clientName}`;
    const folders = await this.sp.web.getFolderByServerRelativePath(clientFolder).folders();
    // Sort versions in descending order
    const versions = folders.map(f => f.Name)
      .sort((a, b) => {
  ...
  public async getExistingStandards(): Promise<IStandard[]> {
    try {
      ...
      const items = await this.sp.web.lists
        ...
        .items
        .select('Id', 'Title',  'FileRef', 'File/Name',
                'UniclassCode1', 'UniclassTitle1', 'UniclassCode2', 'UniclassTitle2',
                'UniclassCode3', 'UniclassTitle3')
                .expand('File')();
      ...
      return items.map(item => ({
        ...item,
        Client: item.FileRef.split('/')[item.FileRef.split('/').indexOf(this.STANDARDS_LIBRARY) + 1],
        Version: item.FileRef.split('/')[item.FileRef.split('/').indexOf(this.STANDARDS_LIBRARY) + 2]
      }))
      .filter(item => !!item.Title && !!item.UniclassCode1 ) as unknown as IStandard[];
    } catch (error) {
  ...
  public async copyExistingStandard(standard: IStandard, projectNumber: string): Promise<void> {
    try {
      ...
      await this.copyToProjectFolder(standard.Title, sourceFolder, uniclassFolder);
  ...
  private async fetchUniclass(context: WebPartContext): Promise<IUniclassItem[]> {
    try {
  ...
  public async mapUniclassCodes(uniclassCode: string, context: WebPartContext): Promise<{
    UniclassTitle1: string;
    UniclassCode1: string;
    UniclassTitle2: string;
    UniclassCode2: string;
  ...
  public async uploadNativeFile(
    ...
    try {
      ...
      const uniclassCodes = await this.mapUniclassCodes(metadata.uniclassCode || '', context);
      await this.documentService.ensureDocumentLibrary(this.STANDARDS_LIBRARY);
      ...
      const fileResult = await this.sp.web.getFolderByServerRelativePath(versionFolder)
  ...
  public async addStandardsToProjectWithProgress(
    files: { fileName: string; uniclassCode: string; client: string; version: string }[],
    projectNumber: string,
    context: WebPartContext,
    sp: SPFI,
    onProgress?: (progress: number, summary: { fileName: string; targetPath: string }[]) => void
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const targetPath = `${siteRoot}/${targetFolder}/${fileName}`.replace(/\\/g, '/').replace(/\/{2,}/g, '/');
