...
import { Dismiss24Regular, Document24Regular, Folder24Regular } from '@fluentui/react-icons';
...
export const StandardsManagementDialog: React.FC<StandardsManagementDialogProps> = ({ isOpen, onDismiss, context, selectedProject, sp }) => {
  const styles = useStyles();
  const standardsService = new StandardsService(context, sp);
  const [currentStep, setCurrentStep] = useState(0);
  const [workflowType, setWorkflowType] = useState<'new' | 'existing' | 'choose'>('choose');
  ...
  // Per-file Combobox query state
  const [queries, setQueries] = useState<string[]>([]);
  const [uploading, setUploading] = useState(false);
  const [uploadProgress, setUploadProgress] = useState(0);
  const [uploadComplete, setUploadComplete] = useState(false);
  // Add for client/version selection in upload step
  const [client, setClient] = useState('');
  const [version, setVersion] = useState('');
  // Add state for copying progress and summary
  const [copying, setCopying] = useState(false);
  const [copyProgress, setCopyProgress] = useState(0);
  const [copySummary, setCopySummary] = useState<{ fileName: string; targetPath: string }[]>([]);
  const [copyComplete, setCopyComplete] = useState(false);

  useEffect(() => {
    async function initialization(): Promise<void> {
        console.log('initialization', selectedProject);
      const next = await standardsService.getNextVersion(selectedProject?.Client || '');
      setVersion(next);
  ...
  const loadAvailableVersions = async (client: string): Promise<void> => {
    try {
      const standards = await standardsService.getExistingStandards();
      const versions = Array.from(new Set(standards
  ...
  const addFiles = (newFiles: File[]): void => {
    setFiles(prev => {
      ...
      const toAdd = newFiles.filter(f => !existingNames.has(f.name)).map(f => ({ name: f.name, file: f }));
      return [...prev, ...toAdd];
    });
  };
  ...
  const getUniclassOptions = (): { children: React.ReactNode; value: string; text: string; title: string }[] => {
    const options: { children: React.ReactNode; value: string; text: string; title: string }[] = [];
    uniclassData.forEach(l1 => {
      options.push({ value: l1.code, children: `${l1.code} - ${l1.title}`, text: `${l1.code} - ${l1.title}`, title: l1.title });
      l1.subgroups?.forEach(l2 => {
        options.push({ value: l2.code, children: `${l2.code} - ${l2.title}`, text: `${l2.code} - ${l2.title}`, title: l2.title });
        l2.sections?.forEach(l3 => {
          options.push({ value: l3.code, children: `${l3.code} - ${l3.title}`, text: `${l3.code} - ${l3.title}`, title: l3.title });
        });
      });
    });
    return options;
  ...
  const goToNext = (): void => setCurrentStep((s) => Math.min(s + 1, steps.length - 1));
  const goToPrev = (): void => {
  ...
  const renderStepContent = (): React.ReactNode => {
    ...
    switch (currentStep) {
            ...
            <ul className={styles.fileList}>
              {files.map((file, idx) => {
                ...
                const options = getUniclassOptions().filter(opt =>
                  String(opt.children).toLowerCase().includes(query.toLowerCase())
                );
                return (
                  <li key={idx} className={styles.fileItem}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
                      <Field label="Title" style={{ minWidth: 200 }}>
                        <input
                          ...
                          onChange={e => {
                            ...
                            setFiles(prev => prev.map((f, i) => i === idx ? { ...f, title: value } : f));
                          }}
                        />
                      </Field>
                      <Field label="Filename" style={{ minWidth: 200 }}>
                        <input
                          ...
                          onChange={e => {
                            const value = e.target.value;
                            setFiles(prev => prev.map((f, i) => i === idx ? { ...f, name: value } : f));
                          }}
                        />
                      </Field>
                      <Field label="Uniclass Code" style={{ minWidth: 320 }}>
                        <Combobox
                          ...
                          onOptionSelect={(_e, data) => {
                            ...
                            const selectedOption = getUniclassOptions().find(opt => opt.value === data.optionValue);
                        ...
                        >
                          {options.length > 0 ? (
                            options.map(opt => (
                              <Option key={opt.value} text={opt.text} value={opt.value}>{opt.children}</Option>
                            ))
                          ) : (
                            <Option disabled>No Uniclass codes match your search.</Option>
                          )}
                        </Combobox>
                      </Field>
                    </div>
          ...
          <div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 16 }}>
              <thead>
                <tr>
                  ...
                  <th style={{ textAlign: 'left', borderBottom: '1px solid #ccc', padding: 8 }}>Uniclass Code</th>
                  <th style={{ textAlign: 'left', borderBottom: '1px solid #ccc', padding: 8 }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {files.map((file, idx) => {
                  const missing = !file.name || !file.title || !file.uniclassCode;
                  return (
                    <tr key={idx} style={{ background: missing ? '#fffbe6' : undefined }}>
                      <td style={{ padding: 8 }}>
                        {file.name || <span style={{ color: 'red' }}>Missing filename</span>}
                      </td>
                      <td style={{ padding: 8 }}>
                        {file.title || <span style={{ color: 'red' }}>Missing title</span>}
                      </td>
                      <td style={{ padding: 8 }}>
                        {file.uniclassCode || <span style={{ color: 'red' }}>Missing Uniclass</span>}
                      </td>
                      <td style={{ padding: 8 }}>
                        <Button size="small" onClick={() => goToPrev()}>Edit</Button>
                        <Button size="small" appearance="subtle" onClick={() => handleRemoveFile(idx)}>
                          Remove
                        </Button>
                      </td>
                    </tr>
            ...
            {files.some(f => !f.name || !f.title || !f.uniclassCode) && (
              <div style={{ color: 'red', marginTop: 12 }}>
                Please ensure all files have a filename, title, and Uniclass code before proceeding.
              </div>
            )}
          </div>
        );
      case 3:
        return (
          <div>
            <Text>Upload validated standards to the central Standards Library:</Text>
            ...
            <ul className={styles.fileList}>
              {files.map((file, idx) => (
                <li key={idx} className={styles.fileItem}>
                  <span><b>{file.name}</b> ({file.title}) - {file.uniclassCode}</span>
                </li>
              ))}
            </ul>
            <div style={{ margin: '16px 0' }}>
              ...
              <div style={{ marginTop: 8 }}>
                {uploading && !uploadComplete && <Text>Uploading... {Math.round(uploadProgress * 100)}%</Text>}
                {uploadComplete && <Text style={{ color: 'green' }}>Upload complete!</Text>}
              </div>
            </div>
            <Button
              appearance="primary"
            ...
            <div style={{ marginTop: 8, color: '#888', fontSize: 13 }}>
              Files will be uploaded to: <b>{selectedProject?.Client || '[Client]'}</b> / <b>{version || '[Version]'}</b>
            </div>
          </div>
        );
      case 4:
        return (
          <div>
            <Text>Copy uploaded standards to the project folder using Uniclass folder structure:</Text>
            ...
            {copyComplete && (
              <div>
                ...
                <ul>
                  {copySummary.map(s => (
                    <li key={s.fileName}>
                      <b>{s.fileName}</b> → <span style={{ fontFamily: 'monospace' }}>{s.targetPath}</span>
                    </li>
                  ))}
                </ul>
              </div>
  ...
  return (
    <Dialog  open={isOpen} onOpenChange={(_, { open }) => !open && handleDismiss()}>
        ...
        <div className={styles.card}>
          ...
          <div className={styles.cardContent}>
            ...
            <div className={styles.navActions}>
              <Button appearance="secondary" onClick={handleDismiss} disabled={uploading}>
                Cancel
              </Button>
              <Button onClick={goToPrev} disabled={currentStep === -1 || uploading}>
                Back
              </Button>
              <Button onClick={goToNext} disabled={currentStep === steps.length - 1 || !canGoNext() || uploading}>
                Next
              </Button>
            </div>
          </div>
