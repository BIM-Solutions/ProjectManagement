...
const DocumentsTab: React.FC<DocumentsTabProps> = ({ context, project }) => {
  ...
  const documentService = useMemo(() => new DocumentService(context), [context]);
  ...
  const getDocumentTemplates = async (): Promise<void> => {
    ...
    setDocumentTypes(templates.map((template: SharePointDocumentTemplate) => ({
  ...
  const getDocumentForType = (docType: string): IDocument | undefined => {
    ...
    return documents.find(doc => doc.UniclassCode3 === mappedDoc.code3 && !doc.IsFolder);
  };
  ...
  const isDocumentAvailable = (docType: string): boolean => {
    ...
    return documents.some(doc => 
      doc.UniclassCode3 === mappedDoc.code3 && !doc.IsFolder
    );
  };

  const getDocumentStatus = (docType: string): { status: 'available' | 'not-available' | 'incorporated', label: string } => {
  ...
  return (
    <>
      <div className={styles.container}>
        <Text size={600} weight="semibold">Key Project Documents</Text>
            ...
            {documentTypes.map((docType) => {
              ...
              return (
                <div key={docType.DocumentType} className={styles.docRow}>
                  <Text className={styles.fieldLabel}>{docType.DocumentType}</Text>
                  ...
                  {status.status === 'available' && (
                    ...
                    <Button
                      ...
                      icon={<Eye24Regular />}
                      onClick={() => {
                        const docToView = getDocumentForType(docType.DocumentType);
                  ...
                  <Badge 
                    ...
                    color={
                      status.status === 'incorporated' ? 'success' :
