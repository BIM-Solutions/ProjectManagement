    ...
    <div className={styles.container}>
          ...
          onSave={async (results: IFilePickerResult[]) => {
            ...
            const upload = await sp.web
              .getFolderByServerRelativePath(
                "/sites/DevelopmentSite/SiteAssets/ProjectImages"
              )
              .files.addUsingPath(result.fileName, arrayBuffer, {
                Overwrite: true,
              });
      ...
      <Field label="Client" required>
        <Input
          value={client}
          onChange={(event) =>
            setClient((event.target as HTMLInputElement).value || "")
          }
          style={{ flexGrow: 1 }}
        />
      </Field>
      ...
      <Field label="Deltek SubCodes" required>
        <Textarea
          value={subCodes}
          onChange={(event) =>
            setSubCodes((event.target as HTMLTextAreaElement).value || "")
          }
          style={{ flexGrow: 1 }}
        />
      </Field>
      <Field label="Client Contact" required>
        <Input
          value={clientContact}
          onChange={(event) =>
            setClientContact((event.target as HTMLInputElement).value || "")
          }
          style={{ flexGrow: 1 }}
        />
      </Field>
